...
가능한 값이 유한하거나 셀 수 있으면 <strong>이산 Random Variable</strong>이다. 주사위의 눈, 성공 횟수, 대기 중인 사람 수가 대표적인 예이다.

가능한 값이 어떤 구간을 연속적으로 채우는 것으로 모델링되면 <strong>연속 Random Variable</strong>이다. 측정된 키, 시간, 온도 등이 이에 해당한다.
